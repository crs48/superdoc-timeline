...
type HostCommandKind = 'text.insert' | 'text.replace' | 'text.deleteBackward' | 'text.deleteForward' | 'text.pastePlain' | 'history.undo' | 'history.redo' | 'structural.enter' | 'structural.listIndent' | 'structural.listOutdent' | string;
...
type HostDispatchResultLike = {
    ...
    readonly status: 'history-committed';
    readonly result?: unknown;
} | {
    readonly status: 'history-noop';
    readonly result?: {
        readonly reason?: string;
    };
} | {
    readonly status: 'receipt-failure';
    readonly failure?: unknown;
} | {
    readonly status: 'rejected';
    readonly rejection: HostDispatchRejectionLike;
};
...
interface DocumentFacadeLike {
    ...
    readonly selection?: {
        current?(input?: {
            includeText?: boolean;
        }): MaybePromise<DocumentFacadeSelectionInfoLike>;
    };
}
type DocumentFacadeResultLike = {
    readonly available: true;
    readonly doc: DocumentFacadeLike;
} | {
...
interface ModeAwareHostLike {
    ...
    save(options?: {
        format?: 'docx';
        commentExportMode?: 'preserve' | 'strip';
    }): Promise<ArrayBuffer>;
...
export declare function createV2EditorRuntimeAdapter(options: V2EditorRuntimeAdapterOptions): {
